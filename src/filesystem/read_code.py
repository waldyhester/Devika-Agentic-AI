...
class ReadCode:
    ...
    def __init__(
        ...
        self.ignore_directories: Set[str] = (
            ...
            else DEFAULT_IGNORE_DIRECTORIES
        )
        self.ignore_files: Set[str] = (
            ignore_files if ignore_files is not None else DEFAULT_IGNORE_FILES
        )
        logger.info(
            f"ReadCode initialized for project: {project_name} at {self.directory_path}"
        )
    ...
    def read_directory(self) -> List[Dict[str, str]]:
        ...
        if not os.path.isdir(self.directory_path):
            logger.warning(
                f"Cannot read directory, path does not exist or is not a directory: {self.directory_path}"
            )
            return files_list
        ...
        for root, dirs, files in os.walk(self.directory_path, topdown=True):
            ...
            dirs[:] = [
                d
                for d in dirs
                if d not in self.ignore_directories and not d.startswith(".")
            ]

            for file_name in files:
                if file_name in self.ignore_files or file_name.endswith(
                    tuple(self.ignore_files)
                ):  # Basic pattern matching for extensions
                    continue

                file_path: str = os.path.join(root, file_name)
                relative_file_path: str = os.path.relpath(
                    file_path, self.directory_path
                )
                ...
                try:
                    with open(
                        ...
                        files_list.append(
                            {"filename": relative_file_path, "code": code}
                        )
        ...
        logger.info(
            f"Read {len(files_list)} files from directory: {self.directory_path}"
        )
        return files_list
    ...
    def code_set_to_markdown(self) -> str:
        ...
        for code_item in code_set:
            ...
            if not lang_indicator:
                ...
                lang_indicator = language_map.get(
                    ext.lower(), ""
                )  # Fallback to empty for unknown
