...
def init_devika() -> None:
    ...
    sqlite_db_dir: str = os.path.dirname(sqlite_db_path) if sqlite_db_path else "" # Handle if path is empty
    ...
    try:
        ...
        if keywords: # Check if keywords were extracted (model likely loaded)
            logger.info(f"BERT model loaded successfully. Test keywords: {keywords[:2]}")
        else:
            logger.info("BERT model loaded, but test keyword extraction returned empty (might be normal for short prompt).")
