...
GIT_EXECUTABLE = config.get_git_executable_path()
DEFAULT_TIMEOUT = config.get_git_default_timeout()
...
class GitService:
    ...
    def __init__(self) -> None:
        ...
        """
        # self.config is no longer needed here if config is module-level and only used for constants.
        # However, get_repos_dir() is an instance method of Config, so we still need an instance.
        # If GIT_EXECUTABLE and DEFAULT_TIMEOUT were the *only* uses, we could pass config around.
        # For now, let's assume other methods might use self.config or it's fine to keep it.
        # The primary goal is to change the module-level constants.
        # Re-evaluating: self.config is used to get self.config.get_repos_dir().
        # So, `self.config = Config()` in __init__ is fine and necessary if other methods use it.
        # The module-level `config` instance is specifically for the module-level constants.
        self.config = Config() # This is fine to keep for instance-specific config needs if any.
        self.repos_base_dir: str = self.config.get_repos_dir()
        os.makedirs(self.repos_base_dir, exist_ok=True)
        logger.info(
            f"GitService initialized. Repositories base directory: {self.repos_base_dir}"
        )
    ...
    def _run_git_command(
        ...
        logger.debug(
            f"Executing Git command: {' '.join(full_command)} in {repo_path or 'current dir'}"
        )
        try:
            ...
            err_msg = (
                f"Unexpected error running Git command {' '.join(full_command)}: {e}"
            )
            logger.error(err_msg, exc_info=True)
            return -1, "", err_msg

    def clone_repo(
        self, url: str, repo_name: str, overwrite: bool = False
    ) -> GitOperationResult:
        """
        ...
        if os.path.exists(repo_path):
            if overwrite:
                logger.info(
                    f"Repository '{repo_name}' already exists at {repo_path}. Overwriting."
                )
                try:
    ...
    def get_current_branch(self, repo_name: str) -> Optional[str]:
        ...
        command = [
            ...
            "--abbrev-ref",
            "HEAD",
        ]  # More reliable than 'git branch --show-current'
        return_code, stdout, stderr = self._run_git_command(
            command, repo_path=repo_path
        )
        ...
        if return_code == 0:
            ...
            logger.error(
                f"Failed to get current branch for '{repo_name}'. Error: {stderr}"
            )
            return None
    ...
    def get_commits(
        ...
        if return_code == 0:
            ...
            logger.info(
                f"Retrieved {len(commits)} commits for '{repo_name}' (branch: {branch or 'current'})."
            )
            return commits
        else:
    ...
    def get_commit_details(
        ...
        if return_code == 0 and stdout.strip():
            ...
            if len(parts) == 9:
                ...
                logger.error(
                    f"Could not parse commit details for {commit_hash} in {repo_name}. Output parts: {len(parts)}"
                )
                return None
        else:
            logger.error(
                f"Failed to get details for commit {commit_hash} in '{repo_name}'. Error: {stderr}"
            )
            return None
    ...
    def get_file_content_at_commit(
        ...
        if return_code == 0:
            ...
            logger.error(
                f"Failed to get file content for '{target}' in '{repo_name}'. Error: {stderr}"
            )
            return None
