...
class NetlifyDeployResult(TypedDict):
    ...
    deploy_url: Optional[str] # This would typically be site.ssl_url or site.url after deploy
    ...
    def deploy_project(self, project_name: str) -> NetlifyDeployResult:
        ...
        if not self.netlify_client:
            return NetlifyDeployResult(
                ...
                site_id=None, deploy_id=None, deploy_url=None, logs_url=None, details=None
            )
        ...
        deploy_logs_url: Optional[str] = None # Usually part of deploy object or constructed
        ...
        try:
            ...
            logger.info(f"Netlify site created. ID: {site_id}, Name: {site_info.get('name')}, URL: {final_site_url}")
            logger.debug(f"Full site creation response: {site_info}")

            if not site_id:
                return NetlifyDeployResult(
                    success=False, message="Failed to create Netlify site: No site ID returned.",
                    site_id=None, deploy_id=None, deploy_url=None, logs_url=None, details=site_info
                )

            # Step 2: Deploy the project directory to the newly created site
            logger.info(f"Deploying project from path: {project_path} to site ID: {site_id}...")
            ...
            deploy_logs_url = deploy_info.get("admin_url", site_info.get("admin_url")) # Fallback to site admin

            logger.info(f"Deployment initiated. Deploy ID: {deploy_id}, State: {deploy_info.get('state')}")
        ...
        except Exception as e: # Catching general exception as netlify_py specific errors are not well documented
            logger.error(f"Netlify deployment failed for project '{project_name}': {e}", exc_info=True)
            return NetlifyDeployResult(
                success=False,
